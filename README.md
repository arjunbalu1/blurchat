...
- **PWA icons (maskable + real sizes)** — the manifest's two icon entries both point at `/logo.png` (a single **1024²** file) declared as 192/512, so the browser just downscales one big image for every slot. Two fixes: (1) ship real `icon-192.png` / `icon-512.png`; (2) add a `purpose: 'maskable'` 512² (logo on a solid bg inside the ~80% **safe zone**) so **Android adaptive icons** fill edge-to-edge instead of a small logo floating on a white rounded square. Also confirm `apple-icon.png` (180²) is **opaque** — iOS fills transparency with black behind its rounded mask. Visible quality issue on Android home-screen installs.
- **Service worker / web push** — none today (Next ships no SW): no offline, no precache, and — the one that matters — **no web push**. A SW is **not** required to install (Chrome dropped that in 2021), and offline is pointless for a real-time chat product, so the *only* reason to add one is **push notifications** ("someone messaged you" while the app is closed). Add via **Serwist** (`@serwist/next`) when push is built; defer until then — a precaching SW serves stale HTML during dev (same hazard as the Cloudflare static-cache TODO below).
